...
module.exports = (() => {
	const config =
	{
		info: {
			...
			version: "0.0.2",
			description: "niggers",
			github: "https://github.com/b3luc/asd/blob/main/nigger.plugin.js",
			github_raw: "https://raw.githubusercontent.com/b3luc/asd/main/nigger.plugin.js"
		},
		defaultConfig: [{
			id: "general",
		...
		const plugin = (Plugin, Api) => { try {
			...
			return class Bruh extends Plugin {
				...
				messageEvent = async ({ channelId, message, optimistic }) => {
					...
					if (!optimistic) {
						const count = (message.content.match(/nigger/gmi) || []).length;
				...
				playBruh() {
					audio.src = "https://cdn.discordapp.com/attachments/869950353192386641/900639871050342430/y2mate.com_-_Keemstar_Nigger_Sound_Effect.mp3";
					audio.play();
				}
